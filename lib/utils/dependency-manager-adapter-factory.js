    ...
    if (config.useWorkspaces) {
      ...
      adapters.push(
        new WorkspaceAdapter({
          ...
          useLockfile: config.useLockfile
        })
      );
    } else if (hasNpm || hasBower) {
      adapters.push(new NpmAdapter({ cwd: root, managerOptions: config.npmOptions, useYarnCommand: config.useYarn, useLockfile: config.useLockfile }));
