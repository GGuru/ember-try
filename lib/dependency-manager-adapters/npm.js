...
module.exports = CoreObject.extend({
  ...
  _install() {
    ...
    const usePureLockFile = mgrOptions.includes('--pure-lockfile');

    /**
     * For yarn, use --pure-lockfile to ignore --no-lockfile
     *
     * For npm, though --pure-lockfile is not a valid option, use it to ignore
     *   --no-shrinkwrap and then remove --pure-lockfile from the options
     */

    if (this.useYarnCommand) {
      if (mgrOptions.indexOf('--no-lockfile') === -1 && !usePureLockFile) {
    ...
    } else if (mgrOptions.indexOf('--no-shrinkwrap') === -1 && !usePureLockFile) {
      mgrOptions = mgrOptions.concat(['--no-shrinkwrap']);
    }

    // Remove --pure-lockfile from npm options as it is only applicable for yarn
    if (cmd === 'npm' && usePureLockFile) {
      mgrOptions = mgrOptions.filter((option) => option !== '--pure-lockfile');
    }
