...
module.exports = CoreObject.extend({
  ...
  _install() {
    ...
    // TODO: Add deprecation warnings accordingly for --no-lockfile

    let cmd = this.useYarnCommand ? 'yarn' : 'npm';
    if (this.useYarnCommand) {
      if (mgrOptions.indexOf('--no-lockfile') === -1 && !this.useLockfile) {
    ...
    } else if (mgrOptions.indexOf('--no-shrinkwrap') === -1 && !this.useLockfile) {
